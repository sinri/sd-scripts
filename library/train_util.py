...
TOKENIZER_PATH_LOCAL = 'E:\\sinri\\HuggingFace\\clip-vit-large-patch14'
if os.path.isdir(TOKENIZER_PATH_LOCAL):
    TOKENIZER_PATH = TOKENIZER_PATH_LOCAL
else:
...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
            self, tokenizer: CLIPTokenizer, max_token_length: int, resolution: Optional[Tuple[int, int]],
            debug_dataset: bool
    ) -> None:
        super().__init__()
    ...
    def get_input_ids(self, caption):
        ...
        if self.tokenizer_max_length > self.tokenizer.model_max_length:
            ...
            if self.tokenizer.pad_token_id == self.tokenizer.eos_token_id:
                ...
                for i in range(
                        1, self.tokenizer_max_length - self.tokenizer.model_max_length + 2,
...
def sample_images(
        accelerator, args: argparse.Namespace, epoch, steps, device, vae, tokenizer, text_encoder, unet,
