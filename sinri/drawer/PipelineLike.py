...
class PipelineLike:
    ...
    def __call__(
        ...
        if not self.token_replacements_XTI:
            text_embeddings, uncond_embeddings, prompt_tokens = PipelineLike.get_weighted_text_embeddings(
                pipe=self,
                prompt=prompt,
        ...
        if negative_scale is not None:
            _, real_uncond_embeddings, _ = PipelineLike.get_weighted_text_embeddings(
                pipe=self,
    ...
    def get_weighted_text_embeddings(
        ...
        if not skip_parsing:
            prompt_tokens, prompt_weights = PipelineLike.get_prompts_with_weights(pipe, prompt, max_length - 2,
                                                                                  layer=layer)
            if uncond_prompt is not None:
                if isinstance(uncond_prompt, str):
                    uncond_prompt = [uncond_prompt]
                uncond_tokens, uncond_weights = PipelineLike.get_prompts_with_weights(pipe, uncond_prompt,
                                                                                      max_length - 2,
                                                                                      layer=layer)
        else:
        ...
        prompt_tokens, prompt_weights = PipelineLike.pad_tokens_and_weights(
            prompt_tokens,
            prompt_weights,
            max_length,
            bos,
        ...
        if uncond_prompt is not None:
            uncond_tokens, uncond_weights = PipelineLike.pad_tokens_and_weights(
                uncond_tokens,
                uncond_weights,
                max_length,
                bos,
        ...
        # get the embeddings
        text_embeddings = PipelineLike.get_unweighted_text_embeddings(
            pipe,
            prompt_tokens,
        ...
        if uncond_prompt is not None:
            uncond_embeddings = PipelineLike.get_unweighted_text_embeddings(
                pipe,
                uncond_tokens,
                pipe.tokenizer.model_max_length,
                clip_skip,
    ...
    def get_prompts_with_weights(pipe: 'PipelineLike', prompt: List[str], max_length: int, layer=None):
        ...
        for text in prompt:
            texts_and_weights = PipelineLike.parse_prompt_attention(text)
            text_token = []
            text_weight = []
    ...
    def parse_prompt_attention(text):
        ...
        for m in PipelineLike.re_attention.finditer(text):
            text = m.group(0)
            weight = m.group(1)
